...
// include: C:\msys64\tmp\tmpqr8f_oda.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'sia-engine.data';
      var REMOTE_PACKAGE_BASE = 'sia-engine.data';
...
Module['FS_createPath']("/", "resources", true, true);
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_sia-engine.data');
      }
      Module['addRunDependency']('datafile_sia-engine.data');
    ...
    loadPackage({"files": [{"filename": "/resources/bullet.glb", "start": 0, "end": 10476}, {"filename": "/resources/cock.wav", "start": 10476, "end": 31630}, {"filename": "/resources/cowboy.glb", "start": 31630, "end": 7687918}, {"filename": "/resources/deagle-2.wav", "start": 7687918, "end": 7756484}, {"filename": "/resources/die2.wav", "start": 7756484, "end": 7800260}, {"filename": "/resources/draw.wav", "start": 7800260, "end": 7840326}, {"filename": "/resources/shader.frag", "start": 7840326, "end": 7840760}, {"filename": "/resources/shader.vert", "start": 7840760, "end": 7841876}, {"filename": "/resources/shader_color.frag", "start": 7841876, "end": 7842127}, {"filename": "/resources/shoot.wav", "start": 7842127, "end": 7910693}, {"filename": "/resources/song.wav", "start": 7910693, "end": 62523733}, {"filename": "/resources/walk.wav", "start": 62523733, "end": 62527101}, {"filename": "/resources/westwood.glb", "start": 62527101, "end": 66273125}], "remote_package_size": 66273125});

  })();

// end include: C:\msys64\tmp\tmpqr8f_oda.js
// include: C:\msys64\tmp\tmpaz1tfdai.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\msys64\tmp\tmpaz1tfdai.js
// include: C:\msys64\tmp\tmpm4grau2i.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\msys64\tmp\tmpm4grau2i.js
...
function findWasmBinary() {
  return locateFile('sia-engine.wasm');
}
...
var ASM_CONSTS = {
  776420: ($0, $1, $2) => { if (window.DeviceOrientationEvent) { window.addEventListener( "deviceorientation", (e) => { setValue($0, e.alpha, 'double'); setValue($1, e.beta, 'double'); setValue($2, e.gamma, 'double'); }, {passive: true, capture:true}, ); } },  
 776653: ($0, $1) => { const configuration = { iceServers: ([{ urls: 'stun:51.21.78.84:3478'}, { urls: 'turn:51.21.78.84:3478', username: 'sia-client', credential: 'chinese-water-deer' }]) }; const pc = new RTCPeerConnection(configuration); dc = null; let ice_candidate_queue = []; pc.onicecandidate = (e) => { if( e.candidate != null && e.candidate.candidate != "" ) { console.log("got candidate: " + JSON.stringify(e.candidate)); if(!pc || !pc.remoteDescription.type) { pc.addIceCandidate(e.candidate).then(() => { console.log("added local ice"); ws.send(e.candidate.candidate); } ); }else { ice_candidate_queue.push(e.candidate); ws.send(e.candidate.candidate); } } }; pc.ondatachannel = (e) => { console.log("got datachannel with binarytype: " + e.channel.binaryType ); dc = e.channel; dc.onmessage = (msg) => { if(e.channel.binaryType == 'blob') { msg.data.bytes().then( (ab) => { writeArrayToMemory(ab, $1); _load_server_state_into_client(); } ); }else if (e.channel.binaryType == 'arraybuffer') { writeArrayToMemory(new Uint8Array(msg.data), $1); _load_server_state_into_client(); } }; }; const ws = new WebSocket("wss://51.21.78.84:27005"); ws.onopen = (event) => { console.log("ws connected"); ws.send("I_WANT_TO_PLAY"); }; var rd_set = false; ws.onmessage = ({data}) => { data.text().then( (text) => { if(text.includes("OFFER")) { let offer = text.split("OFFER:")[1].split(":OFFER")[0]; pc.setRemoteDescription({sdp: offer, type:'offer'}).then( () => { console.log("remote description set:"); rd_set = true; console.log(offer); for (let i = 0; i < ice_candidate_queue.length; i++) { pc.addIceCandidate(ice_candidate_queue[i]).then(() => { console.log("ice: " + JSON.stringify(ice_candidate_queue[i])); } ); } pc.createAnswer().then( answer => { console.log("answer created:" + JSON.stringify(answer)); pc.setLocalDescription(answer).then( () => { let reply = "ANSWER:" + answer.sdp + ":ANSWER"; ws.send(reply); }); } ) } ) } if(text.includes("ICE_CANDIDATE")) { let cand = text.split("ICE_CANDIDATE:")[1].split(":ICE_CANDIDATE")[0]; console.log("Got remote candidate: " + cand); if(rd_set) { pc.addIceCandidate({ candidate: cand, sdpMid: 0, sdpMLineIndex: 0, }); } else{ ice_candidate_queue.push({ candidate: cand, sdpMid: 0, sdpMLineIndex: 0, }); } } if(text.includes("PLAYER_ID")) { var x = Number(text.split("PLAYER_ID:")[1].split(":PLAYER_ID")[0]); console.log("player id: " + x); setValue($0, x, 'i32'); } } ); } },  
 779062: ($0, $1) => { if(dc != null) { ab = new Uint8Array($1); for (let i = 0; i < $1; i++) { ab[i] = getValue($0 + i, 'i8'); } dc.send(ab); } },  
 779188: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 779335: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 779569: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },  
 780121: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },  
 780189: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 781882: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vp', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vp', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },  
 783057: ($0, $1) => { var SDL2 = Module['SDL2']; var numChannels = SDL2.capture.currentCaptureBuffer.numberOfChannels; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(c); if (channelData.length != $1) { throw 'Web Audio capture buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } if (numChannels == 1) { for (var j = 0; j < $1; ++j) { setValue($0 + (j * 4), channelData[j], 'float'); } } else { for (var j = 0; j < $1; ++j) { setValue($0 + (((j * numChannels) + c) * 4), channelData[j], 'float'); } } } },  
 783662: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 784151: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 785157: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 786623: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 787611: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 787694: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 787763: () => { return window.innerWidth; },  
 787793: () => { return window.innerHeight; }
};

// Imports from the Wasm binary.
var _malloc = makeInvalidEarlyAccess('_malloc');
var _load_server_state_into_client = Module['_load_server_state_into_client'] = makeInvalidEarlyAccess('_load_server_state_into_client');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['load_server_state_into_client'] != 'undefined', 'missing Wasm export: load_server_state_into_client');
  ...
  _load_server_state_into_client = Module['_load_server_state_into_client'] = createExportWrapper('load_server_state_into_client', 0);
